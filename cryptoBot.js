    ...
    res.on('end', () => {
      try {
        ...
        if (!parsed.prices || !Array.isArray(parsed.prices)) {
          console.log('⚠️ No price data returned. Possible API issue.');
          return;
        }
        const prices = parsed.prices.map(p => p[1]);
        callback(prices);
      } catch (e) {
        console.log('❌ Failed to parse CoinGecko API response:', e.message);
      }
    });
  }).on('error', err => {
...
fetchPrices(prices => {
  if (!prices || prices.length < 30) {
    console.log('⚠️ Not enough data to perform analysis.');
    return;
  }
...
// Fallback message to ensure output
setTimeout(() => {
  console.log('✅ Crypto bot executed. If no signal appears, check API status.');
}, 4000);
